...
const ESTATUS: Modificador.Tipo = Modificador.Tipo.ESTATUS
...
func test_bonificadores_de_distinto_tipo_se_suman() -> void:
	assert_int(SumaModificadores.total([_m(1, CIRC), _m(2, ESTATUS), _m(1, OBJETO)])).is_equal(4)


func test_penalizadores_del_mismo_tipo_aplica_el_peor() -> void:
	assert_int(SumaModificadores.total([_m(-1, ESTATUS), _m(-2, ESTATUS)])).is_equal(-2)
...
func test_aplicados_para_el_desglose_mantiene_el_orden_y_descarta_el_resto() -> void:
	...
	var estado: Modificador = _m(-1, ESTATUS, "Asustado")
